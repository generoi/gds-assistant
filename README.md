...
Built on [assistant-ui](https://www.assistant-ui.com/) for the chat UI, WordPress 7 AI support and Connectors for availability/credential discovery, and the [WordPress Abilities API](https://github.com/WordPress/abilities-api) for tool execution.
...
Configure provider credentials through WordPress 7 Connectors where available. Legacy environment variables remain supported for the streaming chat providers.

### Environment Variables

Set API keys for the providers you want to use, or configure matching WordPress connector credentials. The chat widget only loads if AI support is enabled and at least one provider is configured.

#### Provider API Keys

Each provider checks WordPress connector credentials first, then legacy env var names:
...
```env
# Anthropic (Claude) — primary provider. Prefer Settings > Connectors on WordPress 7.
